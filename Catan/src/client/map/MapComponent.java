...
public class MapComponent extends JComponent {
	...
	static {
		...
		Point2D cHighPoint = new Point2D.Double(cThreeQuarterBase.getX(),
				cThreeQuarterBase.getY() - CITY_ROOF_HEIGHT);
		CITY.add(cHighPoint);

		VERTEX_POINTS = new HashMap<VertexDirection, Point2D>();
		VERTEX_POINTS.put(VertexDirection.West, new Point2D.Double(
				-HEX_IMAGE_WIDTH / 2, 0));
		VERTEX_POINTS.put(VertexDirection.NorthWest, new Point2D.Double(
				-HEX_IMAGE_WIDTH / 4, -HEX_IMAGE_HEIGHT / 2));
		VERTEX_POINTS.put(VertexDirection.NorthEast, new Point2D.Double(
				HEX_IMAGE_WIDTH / 4, -HEX_IMAGE_HEIGHT / 2));
		VERTEX_POINTS.put(VertexDirection.East, new Point2D.Double(
				HEX_IMAGE_WIDTH / 2, 0));
		VERTEX_POINTS.put(VertexDirection.SouthEast, new Point2D.Double(
				HEX_IMAGE_WIDTH / 4, HEX_IMAGE_HEIGHT / 2));
		VERTEX_POINTS.put(VertexDirection.SouthWest, new Point2D.Double(
				-HEX_IMAGE_WIDTH / 4, HEX_IMAGE_HEIGHT / 2));
	...
	public void addHex(HexLocation hexLoc, HexType hexType) {
		...
		for (VertexDirection vertDir : VertexDirection.values()) {
			VertexLocation vertLoc = new VertexLocation(hexLoc, vertDir)
					.getNormalizedLocation();
	...
	private MouseAdapter mouseAdapter = new MouseAdapter() {
		...
		public void mouseMoved(MouseEvent e) {
			...
			if (dropType == PieceType.ROAD) {
				...
				for (Map.Entry<EdgeLocation, Point2D> entry : allEdgePoints
						.entrySet()) {
				...
				for (Map.Entry<VertexLocation, Point2D> entry : allVertexPoints
						.entrySet()) {
				...
				if (dropType == PieceType.CITY) {
					...
					dropAllowed = getController().canPlaceSettlement(
							dropVertLoc);
				} else {
					assert false;
				}
				...
				for (Map.Entry<HexLocation, Point2D> entry : allHexPoints
						.entrySet()) {
	...
	private void drawSettlements(Graphics2D g2) {
		...
		for (Map.Entry<VertexLocation, CatanColor> entry : settlements
				.entrySet()) {
			VertexLocation vertLoc = entry.getKey();
			CatanColor color = entry.getValue();

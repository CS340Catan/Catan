...
/**
 * Implementation for the player waiting view, which is displayed when the user
 * is waiting for other players to join their game
 */
@SuppressWarnings("serial")
public class PlayerWaitingView extends OverlayView implements
		IPlayerWaitingView {
	...
	public PlayerWaitingView() {
		...
		((JSpinner.DefaultEditor) aiChoices.getEditor()).getTextField()
				.setEditable(false);
	...
	public void setPlayers(PlayerInfo[] value) {
		...
		if (value.length == NUMBER_OF_PLAYERS) {
			...
			labelText = ("Waiting for Players: Need "
					+ (NUMBER_OF_PLAYERS - value.length) + " more");
		...
		for (int i = 0; i < value.length; i++) {
			...
			playerPanel.setLayout(new FlowLayout(FlowLayout.LEFT)); // left
																	// justify
																	// the text
																	// in the
																	// panel
			playerPanel.setPreferredSize(new Dimension(200, 50));
			playerPanel.setBackground(value[i].getColor().getJavaColor()); // set
																			// the
																			// background
																			// color
																			// of
																			// the
																			// player
			JLabel playerLabel = new JLabel(builtString, SwingConstants.LEFT); // justify
																				// the
																				// text
																				// left

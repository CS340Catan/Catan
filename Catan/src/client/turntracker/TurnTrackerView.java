...
/**
 * Implementation for the turn tracker view, which displays whose turn it is,
 * and what state the game is in
 */
...
public class TurnTrackerView extends PanelView implements ITurnTrackerView {
	...
	public TurnTrackerView(TitlePanel titlePanel, GameStatePanel gameStatePanel) {
		...
		longestRoadImage = ImageUtils.loadImage("images/misc/road.png")
				.getScaledInstance(30, 30, Image.SCALE_SMOOTH);
		largestArmyImage = ImageUtils.loadImage("images/misc/army.png")
				.getScaledInstance(30, 30, Image.SCALE_SMOOTH);
	}
	...
	public void initializePlayer(int playerIndex, String playerName,
		...
		playerPanel[playerIndex].add(playerPoints[playerIndex],
				BorderLayout.EAST);

		playerPanel[playerIndex].setBackground(playerColor.getJavaColor());
		playerPanel[playerIndex].setBorder(BorderFactory.createEmptyBorder(3,
				3, 3, 3));
	...
	public void updatePlayer(int playerIndex, int points, boolean highlight,
		...
		if (highlight)
			playerPanel[playerIndex].setBorder(BorderFactory.createLineBorder(
					new Color(0, 0, 0), 3));
		else
	...
	public void updatePlayer(int playerIndex, int points, boolean highlight,
		...
		playerPoints[playerIndex].setText(String.format("%d", points));
		playerPoints[playerIndex].setBackground(playerColor.getJavaColor());

		playerPanel[playerIndex].setBackground(playerColor.getJavaColor());

		for (Component comp : playerPanel[playerIndex].getComponents()) {
			comp.setBackground(playerColor.getJavaColor());
		}

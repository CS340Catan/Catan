...
public class CommandOtherMoveTests {
	...
	public void rollNumberTest() {
		...
		for (Hex hex : FacadeSwitch.getSingleton().getServerModel().getMap()
				.getHexes()) {
			if (hex.getNumber() == 6) {// build a new settlement by a hex with a
										// 6
				VertexObject[] settlements = new VertexObject[1];
				settlements[0] = new VertexObject(playerID, new VertexLocation(
						hex.getLocation(), VertexDirection.East));
		...
		try {
			command.execute();
			assertTrue(player.getResources().totalResourceCount() > 0); // should
																		// get
																		// some
																		// resource
		} catch (ServerResponseException e) {
	...
	public void robPlayerTest() {
		...
		FacadeSwitch.getSingleton().getServerModel().getMap()
				.setSettlements(settlements);
		FacadeSwitch.getSingleton().getServerModel().getPlayers()[3]
				.setResources(new ResourceList(1, 1, 1, 1, 1));
		MoveRobberParams params = new MoveRobberParams(playerID, 3,
				new HexLocation(0, 0));
		Player player = FacadeSwitch.getSingleton().getServerModel()
				.getPlayers()[playerID];
		// should fail and nothing should happen
		...
		try {
			command.execute();
			Player victim = FacadeSwitch.getSingleton().getServerModel()
					.getPlayers()[3];
	...
	public void finishTurnTest() {
		...
		GameList.getSingleton().addGame(
				new GameSummary("test", 0, playerSummaries));
		FacadeSwitch.getSingleton().getServerModel().getTurnTracker()
				.setCurrentTurn(0);
		// should fail and nothing should happen
		...
		try {
			...
			assertTrue(FacadeSwitch.getSingleton().getServerModel()
					.getTurnTracker().getCurrentTurn() == 0);
		}
		...
		try {
			command.execute();
			assertTrue(FacadeSwitch.getSingleton().getServerModel()
					.getTurnTracker().getCurrentTurn() == 3);
		} catch (ServerResponseException e) {
			e.printStackTrace();
			fail("This should work");
		}
	...
	public void discardCardsTest() {
		...
		DiscardCardsParams params = new DiscardCardsParams(playerID,
				resourcesToDiscard);

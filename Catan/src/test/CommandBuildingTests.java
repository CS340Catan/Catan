...
public class CommandBuildingTests {
	...
	public void buildSettlementTest() {
		...
		FacadeSwitch.getSingleton().getServerModel().getMap()
				.setSettlements(blankSettlements);
		BuildSettlementParams params = new BuildSettlementParams(playerID,
				new VertexLocation(new HexLocation(0, 0),
						VertexDirection.NorthEast), false);
		Player player = FacadeSwitch.getSingleton().getServerModel()
				.getPlayers()[playerID];
		Road[] roads = new Road[4];
		roads[0] = new Road(playerID, new EdgeLocation(new HexLocation(0, 0),
				EdgeDirection.North));
		roads[1] = new Road(playerID, new EdgeLocation(new HexLocation(0, 0),
				EdgeDirection.NorthWest));
		roads[2] = new Road(playerID, new EdgeLocation(new HexLocation(0, 0),
				EdgeDirection.SouthWest));
		roads[3] = new Road(playerID, new EdgeLocation(new HexLocation(0, 0),
				EdgeDirection.South));
		...
		FacadeSwitch.getSingleton().getServerModel().getPlayers()[2]
				.setResources(new ResourceList(10, 10, 10, 10, 10));
		// System.out.println("Testing Build Settlement 3");
		try {
			command.execute();
			assertTrue(player.getSettlements() == 4);
			VertexObject[] settlements = FacadeSwitch.getSingleton()
					.getServerModel().getMap().getSettlements();
			assertTrue(settlements[0].getOwner() == playerID);
			assertTrue(settlements[0].getLocation().getHexLoc()
					.equals(new HexLocation(0, 0)));
			assertTrue(settlements[0].getLocation().getDir()
	...
	public void buildCityTest() {
		...
		settlements[0] = new VertexObject(playerID, new VertexLocation(
				new HexLocation(0, 0), VertexDirection.NorthEast));
		FacadeSwitch.getSingleton().getServerModel().getMap()
				.setSettlements(settlements);
		BuildCityParams params = new BuildCityParams(playerID,
				new VertexLocation(new HexLocation(0, 0),
		...
		try {
			...
			VertexObject[] cities = FacadeSwitch.getSingleton()
					.getServerModel().getMap().getCities();
			assertTrue(cities[0].getOwner() == playerID);
			assertTrue(cities[0].getLocation().getHexLoc()
					.equals(new HexLocation(0, 0)));
			assertTrue(cities[0].getLocation().getDir()
					.equals(VertexDirection.NorthEast));
		} catch (ServerResponseException e) {
			e.printStackTrace();
			fail("This should work");
	...
	public void buildRoadTest() {
		...
		try {
			...
			Road[] newRoads = FacadeSwitch.getSingleton().getServerModel()
					.getMap().getRoads();
			assertTrue(newRoads[4].getOwner() == playerID);
			assertTrue(newRoads[4].getLocation().getHexLoc()
					.equals(new HexLocation(0, 0)));
			assertEquals(EdgeDirection.SouthEast, newRoads[4].getLocation()
					.getDir());
		} catch (ServerResponseException e) {
			e.printStackTrace();
			fail("This should work");

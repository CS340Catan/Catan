...
public class CommandTradeNchatTests {
	...
	public void testSendChat() {
		...
		try {
			command.execute();
			assertTrue(FacadeSwitch.getSingleton().getServerModel().getChat()
					.getLines()[0].getMessage().equals("Hi there"));
			assertTrue(FacadeSwitch.getSingleton().getServerModel().getChat()
					.getLines()[0].getSource().equals("Sam"));
		...
		try {
			command.execute();
			assertTrue(FacadeSwitch.getSingleton().getServerModel().getChat()
					.getLines()[1].getMessage().equals("Hey man"));
			assertTrue(FacadeSwitch.getSingleton().getServerModel().getChat()
					.getLines()[1].getSource().equals("Brooke"));
		} catch (ServerResponseException e) {
			e.printStackTrace();
			fail("this should work");
		}
	...
	public void testOfferTrade() {
		...
		try {
			...
			assertTrue(FacadeSwitch.getSingleton().getServerModel()
					.getTradeOffer().getResourceList().getWood() == -1);
			assertTrue(FacadeSwitch.getSingleton().getServerModel()
					.getTradeOffer().getResourceList().getOre() == 0);
			assertTrue(FacadeSwitch.getSingleton().getServerModel()
					.getTradeOffer().getResourceList().getWheat() == 1);
			assertTrue(FacadeSwitch.getSingleton().getServerModel()
	...
	public void testAcceptTrade() {
		/*
		 * Sam: 0, 0, 1, 1, 1 Brooke: 0, 0, 0, 1, 1
		 */
		FacadeSwitch.getSingleton().getServerModel().getTurnTracker()
				.setStatus("Playing");
		// Same gives Brooke 1 wheat for 1 wood
		ResourceList offer = new ResourceList(0, 0, 0, 1, -1);
		TradeOfferParams params = new TradeOfferParams(0, offer, 1);
		command = new OfferTradeCommand(params);
		try {
			command.execute();
		...
		System.out.println("Testing Invalid Accept Trade - Bad Player");
		...
		System.out.println("Testing Valid Accept Trade");
		AcceptTradeParams validUserParams = new AcceptTradeParams(1, true);
		command = new AcceptTradeCommand(validUserParams);
		try {
			command.execute();
			assertTrue(FacadeSwitch.getSingleton().getServerModel()
					.getTradeOffer() == null);
			assertTrue(FacadeSwitch.getSingleton().getServerModel()
					.getPlayers()[0].getResources().equals(
					new ResourceList(0, 0, 1, 0, 2)));
			assertTrue(FacadeSwitch.getSingleton().getServerModel()
					.getPlayers()[1].getResources().equals(
					new ResourceList(0, 0, 0, 2, 0)));
		} catch (ServerResponseException e) {
			fail("Should work");
		}
		...
		FacadeSwitch.getSingleton().getServerModel().getTurnTracker()
				.setStatus("Playing");
		// Same gives Brooke 1 wood for 1 brick (should offer trade, but
		// shouldn't be able to accept)
		offer = new ResourceList(1, 0, 0, 0, -1);
		params = new TradeOfferParams(0, offer, 1);
		command = new OfferTradeCommand(params);
		try {
			command.execute();
			...
			assertTrue(FacadeSwitch.getSingleton().getServerModel()
					.getTradeOffer().getSender() == 0);
			assertTrue(FacadeSwitch.getSingleton().getServerModel()
					.getTradeOffer().getResourceList().getBrick() == 1);
			assertTrue(FacadeSwitch.getSingleton().getServerModel()
					.getTradeOffer().getResourceList().getWood() == -1);
			assertTrue(FacadeSwitch.getSingleton().getServerModel()
					.getTradeOffer().getResourceList().getOre() == 0);
		...
		System.out.println("Testing Invalid Accept Trade - Invalid Resources");
		invalidUserParams = new AcceptTradeParams(1, true);
		command = new AcceptTradeCommand(invalidUserParams);
		try {
			command.execute();
			fail("Executed Invalid Accept Trade Command");
		...
		System.out.println("Testing Reject Trade");
		invalidUserParams = new AcceptTradeParams(1, false);
		command = new AcceptTradeCommand(invalidUserParams);
		try {
			command.execute();
			assertTrue(FacadeSwitch.getSingleton().getServerModel()
	...
	public void testMaritimeTrade() {
		...
		ResourceList resources = FacadeSwitch.getSingleton().getServerModel()
				.getPlayers()[0].getResources();
		resources.setBrick(4);
		resources.setOre(0);
		int oldBankBricks = FacadeSwitch.getSingleton().getServerModel()
				.getBank().getBrick();
		FacadeSwitch.getSingleton().getServerModel().getTurnTracker()
				.setStatus("Playing");

		MaritimeTradeParams params = new MaritimeTradeParams(0, 4, "BRICK",
				"ORE");
		command = new MaritimeTradeCommand(params, 0);

		try {
			command.execute();
			ResourceList newResources = FacadeSwitch.getSingleton()
					.getServerModel().getPlayers()[0].getResources();
			assertTrue(newResources.getBrick() == 0);
			assertTrue(newResources.getOre() == 1);
			int newBankBricks = FacadeSwitch.getSingleton().getServerModel()
					.getBank().getBrick();

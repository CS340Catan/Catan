...
public class ServerModelController {
	...
	public boolean canRollNumber(int playerIndex) {
		if (isPlayerTurn(playerIndex) && model.getTurnTracker().getStatus().equals("Rolling")) {
			return true;
		}
		return false;
	}
	...
	public boolean canBuildRoad(int playerIndex, Road road, boolean isFree, boolean setupPhase) {
		...
		if (isPlayerTurn(playerIndex) && (playerHasResources(playerIndex, requiredResourceList) || isFree) && !roadExists(road)
				&& (hasConnectingBuilding(road) || hasConnectingRoad(road)) && (playerHasAvailableRoadPiece(playerIndex) || setupPhase)
				&& (model.getTurnTracker().getStatus().equals("Playing") || setupPhase) && !roadOnWater(road)) {
			return true;
		}
		return false;
	}
	...
	private boolean roadOnWater(Road road) {
		if (road.getLocation().getHexLoc().isWater() && road.getLocation().getHexLoc().getNeighborLoc(road.getLocation().getDir()).isWater()) {
			return true;
		}
		return false;
	}
	...
	public boolean canBuildSecondRoad(int playerIndex, Road road, boolean setupPhase) {
		...
		if (isPlayerTurn(playerIndex) && !roadExists(road) && (connectedToSecondSettlement(road)) && (playerHasAvailableRoadPiece(playerIndex) || setupPhase)
				&& (model.getTurnTracker().getStatus().equals("Playing") || setupPhase) && !roadOnWater(road)) {
			return true;
		}
		return false;
	...
	public boolean canBuyRoad() {
		...
		ResourceList requiredResourceList = new ResourceList(1, 0, 0, 0, 1);
		if (isPlayerTurn(playerIndex) && playerHasResources(playerIndex, requiredResourceList) && model.getPlayers()[playerIndex].getRoads() > 0)
			return true;
		return false;
	}

	private boolean playerHasResources(int playerIndex, ResourceList resourceList) {
		if (model.getPlayers()[playerIndex].getResources().contains(resourceList)) {
			return true;
		}
		return false;
	}
	...
	private boolean connectedToSecondSettlement(Road road) {
		...
		for (VertexObject settlement : model.getMap().getSettlements()) {
			if (settlement.getOwner() == road.getOwner()) {
				if (buildingExistsForRoad(settlement, road, platformHex) && !roadTouchingNewSettlement(settlement)) {
					return true;
				}
			}
		}
	...
	public boolean canBuyCity() {
		ResourceList resourceList = new ResourceList(0, 3, 0, 2, 0);
		int playerIndex = UserPlayerInfo.getSingleton().getPlayerIndex();
		if (isPlayerTurn(playerIndex) && playerHasResources(playerIndex, resourceList) && model.getPlayers()[playerIndex].getCities() > 0)
			return true;
		return false;
	}
	...
	public boolean canBuildSettlement(VertexObject settlement, boolean isFree, boolean setupPhase) {
		...
		if (isPlayerTurn(playerIndex) && (playerHasResources(playerIndex, resourceList) || isFree) && !preexistingBuilding(settlement, true)
				...
				&& (model.getTurnTracker().getStatus().equals("Playing") || setupPhase) && !settlementOnWater(settlement)) {
			return true;
		}
		return false;
	}
	...
	public ArrayList<ResourceType> getAdjacentResources(VertexObject settlement) {
		...
		case NorthWest:
			...
			location3 = settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.North);
			break;
		case NorthEast:
			location1 = settlement.getLocation().getHexLoc(); 
			location2 = settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.NorthEast);
			location3 = settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.North);
			break;
		case East:
			location1 = settlement.getLocation().getHexLoc(); 
			location2 = settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.NorthEast);
			location3 = settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.SouthEast);
			break;
		case SouthEast:
			location1 = settlement.getLocation().getHexLoc(); 
			location2 = settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.South);
			location3 = settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.SouthEast);
			break;
		case SouthWest:
			location1 = settlement.getLocation().getHexLoc(); 
	...
	private boolean settlementOnWater(VertexObject settlement) {
		switch (settlement.getLocation().getDir()) {
		case NorthWest:
			if (settlement.getLocation().getHexLoc().isWater() && settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.NorthWest).isWater()
					&& settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.North).isWater())
				return true;
			break;
		case NorthEast:
			if (settlement.getLocation().getHexLoc().isWater() && settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.NorthEast).isWater()
					&& settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.North).isWater())
				return true;
			break;
		case East:
			if (settlement.getLocation().getHexLoc().isWater() && settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.NorthEast).isWater()
					&& settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.SouthEast).isWater())
				return true;
			break;
		case SouthEast:
			if (settlement.getLocation().getHexLoc().isWater() && settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.South).isWater()
					&& settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.SouthEast).isWater())
				return true;
			break;
		case SouthWest:
			if (settlement.getLocation().getHexLoc().isWater() && settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.South).isWater()
					&& settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.SouthWest).isWater())
				return true;
			break;
		case West:
			if (settlement.getLocation().getHexLoc().isWater() && settlement.getLocation().getHexLoc().getNeighborLoc(EdgeDirection.NorthWest).isWater()
	...
	public boolean canBuySettlement() {
		ResourceList resourceList = new ResourceList(1, 0, 1, 1, 1);
		int playerIndex = UserPlayerInfo.getSingleton().getPlayerIndex();
		if (isPlayerTurn(playerIndex) && playerHasResources(playerIndex, resourceList) && model.getPlayers()[playerIndex].getSettlements() > 0)
			return true;
		return false;
	}
	...
	public boolean canDiscardCards(int playerIndex) {
		if (model.getPlayers()[playerIndex].getResources().totalResourceCount() > 7 && !model.getPlayers()[playerIndex].alreadyDiscarded()
	 ...
	 * @Post result: a boolean reporting success/fail
	 */
	public boolean canMaritimeTrade(int playerIndex, ResourceType resource, int ratioNumber) {
		if (isPlayerTurn(playerIndex) && model.getPlayers()[playerIndex].getResources().ofAKind(resource, ratioNumber)
				&& model.getTurnTracker().getStatus().equals("Playing")) {
	...
	private boolean playerOnNormalPort(int playerIndex) {
		 ...
		 */
		for (VertexObject settlement : model.getMap().getSettlements()) {
			if (playerIndex == settlement.getOwner() && buildingOnNormalPort(settlement)) {
				return true;
			}
		}
	...
	private boolean playerOnResourcePort(int playerIndex, ResourceType resource) {
		 ...
		 */
		for (VertexObject settlement : model.getMap().getSettlements()) {
			if (playerIndex == settlement.getOwner() && buildingOnResourcePort(playerIndex, settlement, resource)) {
				return true;
			}
		}
		 ...
		 */
		for (VertexObject city : model.getMap().getCities()) {
			if (playerIndex == city.getOwner() && buildingOnResourcePort(playerIndex, city, resource)) {
				return true;
			}
		}
	...
	private boolean buildingOnResourcePort(int playerIndex, VertexObject building, ResourceType resource) {
		 ...
		 */
		for (Port port : model.getMap().getPorts()) {
			if (port.getResource() != null && port.getResource().equals(resource.toString())) {
	...
	public boolean canMoveRobber(HexLocation hexLocation) {
		if (!model.getMap().getRobber().equals(hexLocation) && !hexLocation.isWater()) {
			return true;
		}
		return false;
	}

	public boolean playerTouchingRobber(int robbedPlayer, HexLocation robberLocation) {
		VertexObject testObject = null;
		VertexLocation testLocation = null;
	...
	public boolean canRobPlayer(HexLocation robberLocation, int robbingPlayer, int robbedPlayer) {
		if (isPlayerTurn(robbingPlayer) && model.getTurnTracker().getStatus().equals("Robbing") && playerTouchingRobber(robbedPlayer, robberLocation)
	...
	public boolean canBuyDevCard(int playerIndex) {
		...
		if (isPlayerTurn(playerIndex) && playerHasResources(playerIndex, resourceList) && model.getDeck().hasDevCard()
				&& model.getTurnTracker().getStatus().equals("Playing")) {
			return true;
		}
		return false;
	...
	public boolean canPlaySoldierCard(HexLocation hexLocation, int robbingPlayer, int robbedPlayer) {
		if (isPlayerTurn(robbingPlayer) && model.getPlayers()[robbingPlayer].getOldDevCards().getSoldier() > 0
				&& !model.getPlayers()[robbingPlayer].hasPlayedDevCard() && canRobPlayer(hexLocation, robbingPlayer, robbedPlayer)
	...
	public boolean canPlayYearOfPlentyCard(int playerIndex, ResourceList requestedResources) {
		if (isPlayerTurn(playerIndex) && model.getPlayers()[playerIndex].getOldDevCards().getYearOfPlenty() > 0
				&& !model.getPlayers()[playerIndex].hasPlayedDevCard() && model.getBank().contains(requestedResources)
				&& model.getTurnTracker().getStatus().equals("Playing")) {
			return true;
		}
		return false;
	...
	public boolean canPlayRoadBuildingCard(int playerIndex) {
		if (isPlayerTurn(playerIndex) && model.getPlayers()[playerIndex].getOldDevCards().getRoadBuilding() > 0
				&& !model.getPlayers()[playerIndex].hasPlayedDevCard() && model.getTurnTracker().getStatus().equals("Playing")
	...
	public boolean canPlayMonumentCard(int playerIndex) {
		if (isPlayerTurn(playerIndex) && model.getPlayers()[playerIndex].getOldDevCards().getMonument() > 0
				&& (model.getPlayers()[playerIndex].getVictoryPoints() + model.getPlayers()[playerIndex].getOldDevCards().getMonument()) >= 10
				&& model.getTurnTracker().getStatus().equals("Playing")) {
			return true;
		}
		return false;
	...
	public boolean canPlayMonopolyCard(int playerIndex) {
		if (isPlayerTurn(playerIndex) && model.getPlayers()[playerIndex].getOldDevCards().getMonopoly() > 0
				&& !model.getPlayers()[playerIndex].hasPlayedDevCard() && model.getTurnTracker().getStatus().equals("Playing")) {
			return true;
		}
		return false;

	...
	public void handle(HttpExchange exchange) throws IOException {
		...
		if (playerID == -1) {
			HandlerUtil.sendResponse(exchange, 400,
					"Player Cookie not set. Not logged in!", String.class);
		} else if (gameID == -1) {
			HandlerUtil.sendResponse(exchange, 400, "Game Cookie is not set",
					String.class);
		} else {
			...
			try {
				...
				TradeOfferParams params = (TradeOfferParams) Serializer
						.deserialize(inputStreamString, TradeOfferParams.class);

				FacadeSwitch.getSingleton().setGameID(gameID);
				FacadeSwitch.getSingleton().setPlayerID(playerID);
				ClientModel model = FacadeSwitch.getSingleton().offerTrade(
						params);
